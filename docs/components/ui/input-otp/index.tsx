  ...
  (
        ...
        try {
          ...
          if (clipboardContent.length >= length) {
            ...
            if (pasteValue.length > 0 && mounted) {
              ...
              if (pasteValue.length === length && shouldAutoSubmit && onComplete) {
                if (timerRef.current) clearTimeout(timerRef.current);
                timerRef.current = setTimeout(() => {
    ...
    const handleChange = useCallback(
      (text: string, index: number) => {
        ...
        if (text.length > 1) {
          ...
          let validChars = chars.filter((char, i) => {
          ...
          if (validChars.length > 0) {
            ...
            const nextEmptyIndex = newValue.findIndex((char, i) => !char && i > index);
            if (nextEmptyIndex !== -1) {
              inputRefs.current[nextEmptyIndex]?.focus();
            } else {
              inputRefs.current[Math.min(index + validChars.length, length - 1)]?.focus();
            }
    ...
    const getPositionStyle = useCallback(
      (index: number) => {
        ...
        if (separator && typeof separator !== 'boolean') {
          ...
          if (adjustedIndex === 0) return styles.inputFirstInGroup;
          if (adjustedIndex === midPoint - 1) return styles.inputLastInGroup;
          if (adjustedIndex === midPoint) return styles.inputFirstInGroup;
          if (adjustedIndex === length - 1) return styles.inputLastInGroup;
    ...
    const getInputStyle = useCallback(
      (index: number) => {
        ...
        if (isActive) {
          styleClasses.push(isDark ? styles.inputActiveDark : styles.inputActive);
        }
        ...
        if (hasError) styleClasses.push(isDark ? styles.inputErrorDark : styles.inputError);
    ...
    const errorTextStyle = useMemo(() => {
      return (
        'text-sm text-red-500 mt-2 w-full text-center' + (mode === 'dark' ? ' text-red-400' : '')
      );
    }, [mode]);

    const timerStyle = useMemo(() => {
      return (
        'text-sm text-gray-500 mt-2 w-full text-center' + (mode === 'dark' ? ' text-gray-400' : '')
      );
    }, [mode]);
    ...
    return (
      ...
      >
                  ...
                  <Animated.View style={getAnimationStyle(displayIndex)}>
                    <TextInput
                      ...
                      style={{
                        ...
                        paddingTop: 0,
                        paddingBottom: 0,
                        lineHeight: size === 'sm' ? 16 : size === 'md' ? 18 : 22,
        ...
        {/* Resend Button */}
        {onResend && (
          <View className={`${resendButtonStyle} flex justify-center items-center`}>
            <Button
              onPress={handleResend}
              disabled={resendCountdown > 0 || resendAttempts >= maxResendAttempts}
